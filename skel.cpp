...
void computeHash(const string& hashProgName)
{
	...
	*/
	...
	/* TODO: Send a string to the parent 
	 ...
	 .
	*/
    write(childToParentPipe[WRITE_END], hashValue, strlen(hashValue));
...
void parentFunc(const string& hashProgName)
{
    ...
    close(parentToChildPipe[WRITE_END]); // Close write end
    close(childToParentPipe[READ_END]); // Close read end
